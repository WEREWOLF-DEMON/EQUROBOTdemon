...
@app.on_message(filters.command("msk", prefixes=".") & filters.reply)
async def msk_command(client, message):
    text = message.text.split(maxsplit=1)[1]
    sks = re.findall(r"sk_live_[a-zA-Z0-9]+", text)
    ...
    if sks:
        ...
        final_response = "\n\n".join(responses)
        await message.reply(final_response, parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
    else:
        error_message = "𝗡𝗼 𝘃𝗮𝗹𝗶𝗱 𝗦𝘁𝗿𝗶𝗽𝗲 𝗸𝗲𝘆 𝗳𝗼𝘂𝗻𝗱"
        await message.reply(error_message, parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
