...
def extract_credit_card_details(message_text):
    cards = []
    inputs = re.findall(r"[0-9]+", message_text)

    # Split the inputs into chunks of card details
    chunks = [inputs[i:i + 4] for i in range(0, len(inputs), 4)]
...
async def process_card(ccn, mm, yy, cvv):
    ...
    try:
        ...
        return f"Error during payment method creation: {str(e)}"
    ...
    try:
        ...
        return f"Error during payment confirmation: {str(e)}"
...
@app.on_message(filters.command("xcvv", prefixes=[".", "/"]))
async def check_cc(client, message):
    ...
    if len(cards) > 10:
        return await message.reply_text('Please provide up to 10 cards at a time.')
    ...
    for card in cards:
        ...
        if not (len(ccn) in [13, 15, 16] and len(mm) == 2 and len(yy) in [2, 4] and len(cvv) in [3, 4]):
            results.append(f"Invalid details for card {ccn}.")
            continue

        if not ccn.startswith(VALID_PREFIXES):
            results.append(f"Invalid CC type for card {ccn}.")
            continue

        results.append(await process_card(ccn, mm, yy, cvv))
